...
int MainWindow::certReq(RSA *rsapair)
{
    ...
    len=strlen(bytes);
    entry = X509_NAME_ENTRY_create_by_txt(&entry, "userName", V_ASN1_UTF8STRING, (unsigned char *)bytes,len);
    X509_NAME_add_entry(name, entry, 0, -1);
    strcpy(bytes,ui->lineEdit_5->text().toStdString().c_str());
    certinfo.device = bytes;
    len = strlen(bytes);
    entry = X509_NAME_ENTRY_create_by_txt(&entry, "deviceID", V_ASN1_UTF8STRING, (unsigned char *)bytes, len);
