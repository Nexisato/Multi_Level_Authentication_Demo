...
MainWindow::MainWindow(QWidget *parent)
    ...
    //设置lineEdit正则
    QRegExp regExpIP(
        "((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\d|25[0-4]|[01]?"
        "\\d\\d?)");
    ...
    QRegExpValidator *pRegExpValidatorPort =
        new QRegExpValidator(regExpPort, this);
    QRegExpValidator *pRegExpValidatorReq =
        new QRegExpValidator(regExpReq, this);
    QRegExpValidator *pRegExpValidatorEml =
        new QRegExpValidator(regExpEml, this);
...
void MainWindow::selectFile() {
    ...
    if (!filename.isEmpty()) {
        ...
        ui->textBrowser->append(getTime() +
                                QString("打开文件 '%1' 成功").arg(filename));
    } else {
...
void MainWindow::sendFile() {
    ...
    tcpclient->connectToHost(QHostAddress(ui->lineEdit_9->text()),
                             ui->lineEdit_10->text().toInt());  //连接操作
}
...
void MainWindow::startTransfer() {
    ...
    outblock.resize(0);
    ui->textBrowser->append(getTime() + "总大小：" +
                            QString::number(totalbytes, 10));
    qDebug() << "#####" << totalbytes;
}
...
void MainWindow::updateClientProgress(qint64 numBytes) {
    ...
    {
        ui->textBrowser->append(getTime() +
                                QString("传送文件 '%1' 成功").arg(filename));
        localfile->close();
        tcpclient->close();
    }
}
...
void MainWindow::displayError(QAbstractSocket::SocketError) {
    ui->textBrowser->append(
        getTime() + "发生错误: " + tcpclient->errorString() + "... 操作取消");
    tcpclient->close();
...
//点击[选择文件]按钮事件
void MainWindow::on_pushButton_4_clicked() { selectFile(); }
...
void MainWindow::on_pushButton_clicked() {
    ...
    if (ret1 != 1) {
        ui->textBrowser->append(
            getTime() + "生成证书请求文件失败，请更新版本或联系开发人员");
    } else {
