...
void MainWindow::acceptConnection() {
    ...
    connect(tcpserconn, SIGNAL(readyRead()), this,
...
void MainWindow::updateServerProgress() {
    ...
    if (bytesrecved <=
        sizeof(qint64) *
            2) {  //如果接收到的数据小于16个字节，那么是刚开始接收数据，我们保存到//来的头文件信息
        ...
        if ((tcpserconn->bytesAvailable() >= filenamesize) &&
            ...
            if (exist) {
                ui->textBrowser->append(
                    getTime() + "附件文件夹已存在，文件已存入" + tmpdirname);
            ...
            ui->textBrowser->append(
                getTime() + QString("正在接收文件 '%1' ...").arg(filename));
    ...
    if (bytesrecved < totalbytes) {  //如果接收的数据小于总数据，那么写入文件
    ...
    //更新进度条
    if (bytesrecved == totalbytes) {  //接收数据完成时
        tcpserconn->close();
        localfile->close();
        ui->pushButton->setEnabled(true);
        ui->textBrowser->append(getTime() +
                                QString("接收文件 '%1' 成功！").arg(filename));
        //更新treeview
